...
def extract_ibi_hard_code(in_path, out_path, filename):
    info_path = in_path / "participant_info.csv"
    ...
    try:
        ...
        # Construct the expected output path from the input filename
        out_filename = filename.name.split("_")[0] + ".npz"
        out_path = out_path / out_filename

        # Correctly check if the OUTPUT file already exists
        if not out_path.exists():
            sid = filename.name.split("_")[0]
            print(sid)
            AHI = info_df.loc[info_df["SID"] == sid, "AHI"].values[0] # pyright: ignore
            print("AHI: ", AHI)

            ibi, fs, stages = read_dreamt_data(
                filename,  # Pass the Path object directly
                select_chs=["BVP"],
                fixed_fs=100,
                epoch_duration=30,
                use_package="neurokit",
...
if __name__ == "__main__":
    ...
    IN_PATH = ROOT_PATH / "data/raw/DREAMT"
    OUT_PATH = ROOT_PATH / "data/processed/DREAMT_PIBI_SE"
    for file in tqdm((IN_PATH / "data_100Hz").iterdir(), desc="Processing DREAMT files"):
        extract_ibi_hard_code(IN_PATH, OUT_PATH, file)
